...
<div class="container">
    ...
    <div class="grid">
        ...
        <div class="card">
            <h2 style="margin-top:0">الدخول</h2>
            <div class="row">
                <div>
        ...
        <div class="card">
            <h2 style="margin-top:0">إنشاء مهمة</h2>
            <div class="row">
                <div>
                    <label for="newTitle">العنوان</label>
                    <input id="newTitle" type="text" placeholder="عنوان المهمة" />
                </div>
                <div>
                    <label for="newDesc">الوصف</label>
                    <input id="newDesc" type="text" placeholder="وصف مختصر (اختياري)" />
                </div>
            </div>
            <div class="actions" style="margin-top:10px">
                <button id="createTask">إضافة مهمة</button>
                <span id="createStatus" class="status"></span>
            </div>
        </div>

        <div class="card">
            <h2 style="margin-top:0">قائمة المهام</h2>
            <div class="actions" style="margin-bottom:8px">
                <button id="refresh" class="outline">تحديث القائمة</button>
...
(() => {
    ...
    async function login() {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error('فشل الدخول: ' + res.status + ' ' + text);
            }
            const data = await res.json();
            setStatus(authStatus, 'تم الدخول.');
    ...
    async function logout() {
        ...
        try {
            ...
            if (!res.ok) {
                const text = await res.text();
                throw new Error('فشل الخروج: ' + res.status + ' ' + text);
            }
            setStatus(authStatus, 'تم الخروج.');
    ...
    async function register() {
        ...
        try {
            ...
            if (!res.ok) {
                const text = await res.text();
                throw new Error('فشل التسجيل: ' + res.status + ' ' + text);
            }
            setStatus(regMsg, 'تم إنشاء الحساب. يمكنك تسجيل الدخول الآن.');
        } catch (e) {
            console.error(e);
    ...
    async function loadUser() {
        try {
            ...
            userInfo.textContent = `مسجل باسم: ${escapeHtml(user.name || '')} — ${escapeHtml(user.email || '')}`;
            logoutBtn.disabled = false;
        } catch (e) { console.error(e); }
    }

    async function loadTasks() {
        setStatus(listStatus, 'جارٍ تحميل المهام...');
        tasksEl.innerHTML = '';
        try {
            const res = await fetch('/api/tasks', { headers: { 'Accept': 'application/json' }, credentials: 'same-origin' });
            if (!res.ok) {
                const text = await res.text();
                throw new Error('فشل التحميل: ' + res.status + ' ' + text);
            }
            const tasks = await res.json();
            if (!Array.isArray(tasks) || tasks.length === 0) {
                tasksEl.innerHTML = '<li class="empty">لا توجد مهام بعد</li>';
    ...
    function renderTaskItem(t) {
        ...
        li.innerHTML = `
            ...
            <div class="actions">
                <button class="toggle">${t.completed ? 'إلغاء الإكمال' : 'تحديد كمكتمل'}</button>
    ...
    async function deleteTask(id, li) {
        ...
        try {
            ...
            if (!res.ok) {
                const text = await res.text();
                throw new Error('فشل الحذف: ' + res.status + ' ' + text);
            }
            li.remove();
    ...
    async function toggleTask(id, li) {
        ...
        try {
            ...
            if (!res.ok) {
                const text = await res.text();
                throw new Error('فشل التبديل: ' + res.status + ' ' + text);
            }
            const updated = await res.json();
            li.classList.toggle('done', !!updated.completed);
            li.querySelector('.meta').textContent = updated.completed ? 'مكتملة' : 'غير مكتملة';
            li.querySelector('.toggle').textContent = updated.completed ? 'إلغاء الإكمال' : 'تحديد كمكتمل';
        } catch (e) { console.error(e); alert(e.message); }
    }
    ...
    async function createTask() {
        ...
        try {
            ...
            if (!res.ok) {
                const text = await res.text();
                throw new Error('فشل الإنشاء: ' + res.status + ' ' + text);
            }
            const created = await res.json();
            tasksEl.prepend(renderTaskItem(created));
            newTitle.value = '';
            newDesc.value = '';
            setStatus(createStatus, 'تمت الإضافة.');
